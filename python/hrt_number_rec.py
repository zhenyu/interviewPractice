...
class  Solution(object):
    def num_of_rec(self, xs:List[int], ys:List[int]):
        ret =0
        p_dist_map = {}
        points =zip(xs, ys)
        ...
        for p0_index in range(len(points)-3):
            for p1_index in range(p0_index+1, len(points)-2):
                d01=self.dist(points[p0_index], points[p1_index])
                if d01>0:
                    for p2_index in range(p1_index+1, len(points)-1):
                        d02= self.dist(points[p0_index], points[p2_index])
                        if d02>0:
                            p3s1=p_dist_map.get(p1_index).get(d02, [])
                            p3s2=p_dist_map.get(p2_index).get(d01, [])
...
if __name__ == '__main__':
    print(Solution().num_of_rec([0,0,1,1], [0,2,0,4]))